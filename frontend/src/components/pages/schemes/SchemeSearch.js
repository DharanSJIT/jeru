    ...
    const SelectField = ({ label, name, value, onChange, options, icon: Icon }) => (
        <div className="relative">
            <select
                ...
                value={value}
                onChange={onChange}
                className="w-full p-2.5 pl-10 pr-12 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#0052CC] focus:border-transparent transition-all duration-200 appearance-none text-gray-700 focus:outline-none text-sm"
                style={{ maxWidth: '100%' }}
            >
            ...
            <div className="max-w-7xl mx-auto px-4 py-4">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="relative max-w-3xl mx-auto">
                        <div className="flex items-center bg-white border-2 border-[#0052CC] rounded-xl overflow-hidden shadow-sm focus-within:shadow-md transition-shadow duration-200">
                            <input
                                type="text"
                                name="search"
                            ...
                            <button
                                type="submit"
                                className="px-4 sm:px-6 py-3 bg-[#0052CC] text-white hover:bg-[#003D99] transition-colors duration-200 flex items-center gap-2 flex-shrink-0"
                            >
                                <Search size={18} />
                        ...
                        <div className="bg-white rounded-lg p-3 shadow-sm border border-gray-100">
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(filters).map(([key, value]) => 
                                    value && (
                                        <div key={key} 
                                            className="flex items-center gap-1.5 bg-[#0052CC] bg-opacity-10 text-[#0052CC] px-2 py-0.5 rounded-md text-sm font-medium">
                                            {key}: {value}
                                            <button
                                                type="button"
                    ...
                    {/* Filter Tabs */}
                    <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-[#0052CC]">
                            ...
                            {tabs.map(tab => (
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-3 sm:px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-colors duration-200 flex-1
                                        ${activeTab === tab.id 
                                            ? 'text-[#0052CC] border-b-2 border-[#0052CC] bg-blue-50' 
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={handleFilterChange}
                                            className="w-full p-[.430rem] pl-10 pr-4 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#0052CC] focus:border-transparent transition-all duration-200"
